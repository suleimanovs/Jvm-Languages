...
/**
 * Это вложенный класс, или как говорят [Nested] класс. Такой запись тоесть обьявленние
...
class OuterNested {
    ...
    class Nested {
         ...
         * для OuterNested как static.Для доступа к этому классу мы не обьязаны создать
         * объект [OuterNested]
         */
        fun foo() = 2
    }
...
/**
 * Это внутренний класс, или как говорят [Inner] класс. Такой запись тоесть обьявленние
...
class OuterInner {
    ...
    inner class Inner {
        /**
         * Тут мы можем обращаться к свойству [bar], потому что этот класс
         * для OuterInner внутренний, это часть его. Для доступа к этому классу мы должны создать
         * объект [OuterInner]
         */
        fun foo() = bar
    }
